...
from lab2.schemas import Us_Sch, Categ_Sch, Rec_Sch, Curr_Sch
...
def create_user():
    ...
    if default_currency_id is None:
        ...
        if not default_currency:
            # Default currency does not exist, so create it
    ...
    with app.app_context():
        ...
        if user:
            if provided_user_id is not None and username == user.username:
                ...
                return jsonify({"message": "unsuccessful login (invalid user ID or username)"}), 401
        else:
